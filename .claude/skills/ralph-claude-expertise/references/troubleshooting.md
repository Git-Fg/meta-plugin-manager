...
# Fix: Return value from function being tested
...
error: unused variable `result`
 --> src/lib.rs:42:9
  |
42 |     let result = process();
  |         ^^^^^^ help: if this is intentional, prefix it with an underscore: `_result`

# Fix: Use the variable or prefix with underscore

warning: consider implementing `Display` for `Error`
# Fix: Implement Display trait or use derive macro
```
...
#### Symptom: Same-iteration hat switching (Multiple hats in one iteration)
**Root Cause**: Hat prompt missing STOP instruction, or events not being read from JSONL
**Diagnosis**:
```bash
...
**Fix checklist:**
1. Check workflow prompt has "CRITICAL: STOP after publishing"
2. Check DELEGATE section is clear about yielding control
3. Verify `HatInfo` includes `instructions` field
4. Ensure `## PENDING EVENTS` section is in prompt
