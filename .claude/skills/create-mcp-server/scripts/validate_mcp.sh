...
Validates .mcp.json configuration and reports quality score.
...
for name, server in servers.items():
    ...
    if 'transport' not in server:
        issues.append(f"{name}: Missing 'transport'")
        score -= 10
        continue
    ...
    if transport_type == 'stdio':
        ...
        if 'command' not in transport:
            issues.append(f"{name}: Missing 'command' for stdio")
            score -= 5
        if 'args' not in transport:
            warnings.append(f"{name}: Missing 'args' for stdio")
            score -= 2

    elif transport_type == 'streamable-http':
        # Validate streamable-http configuration
        if 'url' not in transport:
            issues.append(f"{name}: Missing 'url' for streamable-http")
            score -= 5

    else:
        issues.append(f"{name}: Unknown transport type '{transport_type}'")
        score -= 10
