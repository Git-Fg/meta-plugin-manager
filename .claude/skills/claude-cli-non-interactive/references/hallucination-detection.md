...
**Use the `tool-analyzer` skill instead of manual verification**

The `tool-analyzer` skill automatically performs all verification steps:
...
**Call the skill**:
```bash
# The tool-analyzer will analyze test-output.json and provide:
...
## Manual Verification (if tool-analyzer unclear)
