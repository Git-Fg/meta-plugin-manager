...
```bash
curl -X DELETE https://jules.googleapis.com/v1alpha/sessions/abc123 \
  -H "X-Goog-Api-Key: YOUR_API_KEY"
```
...
| `pageToken` | string | Token from previous call for pagination |
| `createTime` | string | **(New Jan 2026)** Filter activities created after this timestamp (RFC 3339 format). Use as a range cursor to fetch only new activities. |
...
# Filter activities created after a specific timestamp (Jan 2026 feature)
ENDPOINT="https://jules.googleapis.com/v1alpha/sessions/abc123/activities"
TIMESTAMP="2026-01-17T00:03:53.137240Z"
curl "$ENDPOINT?createTime=$TIMESTAMP" \
  -H "X-Goog-Api-Key: YOUR_API_KEY"
```
