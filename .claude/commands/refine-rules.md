---
name: refine-rules
description: Analyze all CLAUDE.md and .claude/rules files for quality, consistency, and best practices. Identify gaps, redundancies, and alignment issues. Generate actionable refinement recommendations with priority levels.
...
Use `/refine-rules` when:
- Setting up a new project's memory structure
- After significant project evolution or refactoring
- When Claude consistently ignores or misinterprets rules
- Before major documentation updates
- As periodic maintenance (recommended quarterly)
...
- Use `/verify` to ensure changes don't break existing workflows
