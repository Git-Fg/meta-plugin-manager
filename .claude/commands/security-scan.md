---
name: security-scan
description: "Execute predefined security scans and display results. Use when: user mentions '/security-scan', '/scan-security', 'quick security check'; performing routine security audits; getting rapid vulnerability assessment. Integrates with security-auditor agent for comprehensive analysis. Not for: deep security reviews (use agent directly)."
user-invocable: true
---
...
**Command**: `/security-scan` or `/scan-security`
...
**Command**: `/security-scan comprehensive` or `/scan-security --comprehensive`
...
### 3. Targeted Scan
**Command**: `/security-scan --target [pattern]` or `/scan-security --target [pattern]`
**Scope**: Focus on specific files, directories, or vulnerability types
**Duration**: Variable
**Output**: Focused findings on target area

**Examples:**
- `/security-scan --target "*.js"` - Scan JavaScript files only
- `/security-scan --target "auth"` - Focus on authentication-related code
- `/security-scan --target "api"` - Focus on API endpoints
...
### Manual Delegation
**Command**: `/security-scan --deep` or `/scan-security --agent`
