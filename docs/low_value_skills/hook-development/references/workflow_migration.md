---
description: "Migration guide for converting basic command hooks to advanced prompt-based hooks. Use when upgrading hooks for better maintainability, context awareness, and LLM reasoning. Includes before/after examples and checklist."
---
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Command: $TOOL_INPUT.command. Analyze for: 1) Destructive operations (rm -rf, dd, mkfs, etc) 2) Privilege escalation (sudo) 3) Network operations without user consent. Respond with JSON: {\"ok\": true} to allow, or {\"ok\": false, \"reason\": \"explanation\"} to block.",
          "timeout": 15
        }
      ]
    }
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "File path: $TOOL_INPUT.file_path. Content preview: $TOOL_INPUT.content (first 200 chars). Verify: 1) Not system directories (/etc, /sys, /usr) 2) Not credentials (.env, tokens, secrets) 3) No path traversal 4) Content doesn't expose secrets. Respond with JSON: {\"ok\": true} to allow, or {\"ok\": false, \"reason\": \"explanation\"} to block."
        }
      ]
    }
  ]
