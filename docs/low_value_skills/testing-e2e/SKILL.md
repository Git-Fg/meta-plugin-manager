---
name: testing-e2e
description: "Execute programmatic E2E tests for all .claude/ components. Use when validating skills, commands, agents, hooks, or MCP servers. Not for unit tests or CI/CD pipelines. Keywords: e2e test, end-to-end, component validation, skill test, command test, agent test, hook test, mcp test, sandbox, claude -p"
---
...
| Question | Check |
| :--- | :--- |
| Sandbox created? | `.sandbox/.claude/` structure mirrors project |
| CWD correct? | Inside `.sandbox/.claude/` for claude -p |
| Scenarios pre-created? | No on-the-fly creation during tests |
| Logs permanent? | Not /tmp or /dev/null |
| Hallucination detected? | Scenarios included in tests |
| Exit codes checked? | All commands validated |
