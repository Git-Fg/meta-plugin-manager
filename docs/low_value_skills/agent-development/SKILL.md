...
**Review agent:** See ## Validation Checklist → Apply all quality gates
...
**If you're wondering why this skill is so detailed:** Agents are expensive to create—mistakes here waste significant time and tokens.
...
| Review agent quality         | ## ANTI-PATTERN: Common Mistakes       |
...
**This skill (agent-development) is a meta-skill**—a skill about creating agents. Meta-skills warrant full structure because they teach patterns that agents will apply repeatedly.

### When to Use Full Structure

Full structure (patterns, examples, anti-patterns) is warranted when:

| Agent Type | Why Full Structure |
| :--- | :--- |
| **Meta-agents** | Teaches patterns reused across many agents |
| **Complex orchestration** | Multi-step workflows where agents cut corners |
| **Production agents** | High-cost operations where errors are expensive |
| **Low-freedom domains** | Precise workflows where deviation breaks things |

### When to Use Light Structure

Simple domain agents can be much leaner:

```yaml
---
name: simple-validator
description: "Validate JSON files. Use when checking schema compliance."
---

## Task

Validate JSON file against schema. Report: PASS/FAIL with errors.

**10 lines. Complete. Sufficient.**
...
- The cost of getting it wrong is high (fork isolation is expensive)
- The pattern will be applied many times
- Edge cases are non-obvious (context fork, philosophy bundle)
- Portability requires self-containment
...
| :----------------------------- | :--------------------------------------- | :----------------------------------------- |
| **Copy-paste agent patterns**  | `references/pattern_agent_templates.md`      | `Read` to copy system prompt templates     |
...
| **Philosophy bundle** | Behavioral guidance for context fork | `<philosophy_bundle>` |
| **Non-negotiable rules** | Separates constraints from guidance | `<critical_constraint>` |
| **Interaction schema** | Multi-step workflow definition | `<interaction_schema>` |

### When to Skip XML

| Case | Use Instead |
| :--- | :--- |
| Simple examples | Markdown code blocks |
...
- Simple examples? No → Use markdown
