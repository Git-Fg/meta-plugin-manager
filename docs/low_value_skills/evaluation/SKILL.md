...
**If you need to create evaluation rubric:** Design 4-6 weighted dimensions → Implement LLM judge → Build test set → Set threshold

**If you need to measure quality:** Apply multi-dimensional rubric → Document evidence for each score → Calculate weighted overall

**If you need quality gate:** Set threshold (≥0.7 production, ≥0.6 experimental) → Block substandard work → Track over time

## Navigation

| If you need... | Read... |
| :--- | :--- |
| Evaluation principles | ## PATTERN: Evaluation Principles |
| Rubric design | ## PATTERN: Multi-Dimensional Rubric |
| LLM judge implementation | ## PATTERN: LLM-as-Judge |
| Quality gate setup | ## PATTERN: Quality Gate |
| Common mistakes | ## ANTI-PATTERN: Common Mistakes |
| Troubleshooting | ## EDGE: Troubleshooting |
| Self-check | ## Recognition Questions |

## PATTERN: Evaluation Principles
...
| Question | Check |
| :--- | :--- |
| Rubric multi-dimensional? | 4-6 dimensions with weights |
| Evidence documented? | Each score has file:line reference |
| LLM judge includes all dimensions? | Rubric fully covered in prompt |
| Position swapping implemented? | Bias reduced in comparisons |
| Threshold appropriate? | ≥0.7 production, ≥0.6 experimental |
| Test set stratified? | Simple, moderate, edge cases included |
...
- "Did I read the actual file?" → Verify before claiming
</critical_constraint>
