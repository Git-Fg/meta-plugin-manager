...
**If you're wondering why this skill is so detailed:** Commands are used repeatedly—strong guidance prevents common mistakes that waste time.
...
**This skill (command-authoring) is a meta-skill**—a skill about creating commands. Meta-skills warrant full structure because they teach patterns that agents will apply repeatedly.

### When to Use Full Structure

Full structure (examples, tables, anti-patterns) is warranted when:

| Component Type | Why Full Structure |
| :--- | :--- |
| **Meta-skills** | Teaches patterns reused across many commands |
| **Complex commands** | Multi-step workflows where agents cut corners |
| **Security-sensitive** | Destructive operations where errors are costly |
| **Low-freedom domains** | Precise patterns where deviation breaks things |

### When to Use Light Structure

Domain-specific commands can be much leaner:

```markdown
---
description: "Format timestamp as ISO 8601 UTC"
---
...
- The cost of getting it wrong is high (rm -rf, sudo)
- The pattern will be applied many times
- Edge cases are non-obvious (@/! injection behavior)
- Portability requires clear structure
...
| :--- | :--- | :--- |
| **Injected content** | Semantic grouping for file injection | `<injected_content>` |
...
- File injection semantics? Yes → Use `<injected_content>`
...
✅ Correct (skill):
Read the configuration section below to understand the schema.
```
