...
const AskPerplexityWithAssetsSchema = z.object({
    ...
    assets: z
        ...
        data: z
            .string()
            .describe("Asset as file path (e.g., /path/to/file.pdf or ./docs/doc.png), URL (http:// or https://), or base64-encoded content"),
    }))
        .optional()
        .default([])
...
export function registerPerplexityTools(server) {
    server.registerTool("ask_perplexity", {
        ...
        description: "Tool to query Perplexity AI for web-grounded answers with citations. Use when you need research-focused responses with sources. Constraint: use search_perplexity for raw search results; use sonar-pro for advanced search.",
    ...
    server.registerTool("search_perplexity", {
        ...
        description: "Tool to get ranked web search results with real-time information. Use when you need raw search results without AI summarization. Constraint: supports multi-query, domain filtering, date filtering, and search mode (web/academic/sec).",
    ...
    server.registerTool("ask_perplexity_with_assets", {
        ...
        description: "Tool to query Perplexity with image or document attachments for web-grounded multimodal analysis. Use when you need to analyze visual or document content with web context. Constraint: supports file paths (absolute or relative), URLs, and base64; max 50MB per file.",
