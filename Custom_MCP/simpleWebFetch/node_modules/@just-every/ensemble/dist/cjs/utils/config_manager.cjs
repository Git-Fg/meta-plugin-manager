...
const errors_js_1 = require("../types/errors.cjs");
    ...
    loadConfig() {
        ...
        if (process.env.ENSEMBLE_TOOL_TIMEOUT) {
            ...
            if (isNaN(timeout) || timeout <= 0) {
                throw new errors_js_1.ConfigurationError('ENSEMBLE_TOOL_TIMEOUT must be a positive number', {
        ...
        if (process.env.ENSEMBLE_MAX_CONCURRENT_TOOLS) {
            ...
            if (isNaN(maxTools) || maxTools <= 0) {
                throw new errors_js_1.ConfigurationError('ENSEMBLE_MAX_CONCURRENT_TOOLS must be a positive number', {
        ...
        if (process.env.ENSEMBLE_PAUSE_CHECK_INTERVAL) {
            ...
            if (isNaN(interval) || interval <= 0) {
                throw new errors_js_1.ConfigurationError('ENSEMBLE_PAUSE_CHECK_INTERVAL must be a positive number', {
        ...
        if (process.env.ENSEMBLE_HISTORY_COMPACTION_THRESHOLD) {
            ...
            if (isNaN(threshold) || threshold <= 0 || threshold > 1) {
                throw new errors_js_1.ConfigurationError('ENSEMBLE_HISTORY_COMPACTION_THRESHOLD must be a number between 0 and 1', {
