...
exports.processToolResult = processToolResult;
exports.shouldSummarizeResult = shouldSummarizeResult;
exports.getTruncationMessage = getTruncationMessage;
exports.clearSummaryCache = clearSummaryCache;
exports.getSummaryCacheStats = getSummaryCacheStats;
...
async function createSummary(content, prompt, agent) {
    ...
    try {
        ...
        const { ensembleRequest } = await Promise.resolve().then(() => __importStar(require("../core/ensemble_request.cjs")));
        const messages = [
            {
                type: 'message',
                role: 'system',
        ...
        const truncated = truncate(content, tool_execution_js_1.MAX_RESULT_LENGTH);
...
async function createExpandableSummary(content, prompt, agent) {
    ...
    const hashMapPath = path_1.default.join(summariesDir, HASH_MAP_FILENAME);
    const documentHash = crypto_1.default.createHash('sha256').update(content).digest('hex');
    const hashMap = await loadHashMap(hashMapPath);
    if (hashMap[documentHash]) {
        const summaryId = hashMap[documentHash];
        const summaryFilePath = path_1.default.join(summariesDir, `summary-${summaryId}.txt`);
        const originalFilePath = path_1.default.join(summariesDir, `original-${summaryId}.txt`);
        try {
    ...
    try {
        ...
        const summaryFilePath = path_1.default.join(summariesDir, `summary-${newSummaryId}.txt`);
        const originalFilePath = path_1.default.join(summariesDir, `original-${newSummaryId}.txt`);
        try {
            await Promise.all([
...
async function injectSummaryTools(agent) {
    ...
    const { getSummaryTools } = await Promise.resolve().then(() => __importStar(require("./summary_utils.cjs")));
...
async function processToolResult(toolCall, rawResult, agent, allowSummary) {
    const toolName = toolCall.function.name;
    const config = tool_execution_js_1.TOOL_CONFIGS[toolName] || {};
    ...
    const skipSummarization = config.skipSummarization || tool_execution_js_1.SKIP_SUMMARIZATION_TOOLS.has(toolName);
    const maxLength = config.maxLength || tool_execution_js_1.MAX_RESULT_LENGTH;
...
function shouldSummarizeResult(toolName, resultLength) {
    const config = tool_execution_js_1.TOOL_CONFIGS[toolName] || {};
    if (config.skipSummarization || tool_execution_js_1.SKIP_SUMMARIZATION_TOOLS.has(toolName)) {
        return false;
    }
    const maxLength = config.maxLength || tool_execution_js_1.MAX_RESULT_LENGTH;
    return resultLength > maxLength;
}
function getTruncationMessage(toolName) {
    const config = tool_execution_js_1.TOOL_CONFIGS[toolName] || {};
    return config.truncationMessage || `... Output truncated to ${config.maxLength || tool_execution_js_1.MAX_RESULT_LENGTH} characters`;
}
