...
exports.bytedanceProvider = exports.ByteDanceProvider = void 0;
...
function mapSize(size) {
    ...
    if (s === 'portrait')
        return '1024x1536';
    if (s.toUpperCase() === '2K' || s.toUpperCase() === '4K' || s.toUpperCase() === '720P' || s.toUpperCase() === '1080P')
        return s;
    if (/^\d+x\d+$/i.test(s))
        return s;
    return undefined;
...
function normalizeModelId(model) {
    if (model === 'seedream-4' || model === 'seedream-4.0')
        return 'seedream-4-0-250828';
    return model.replace(/^bytedance[\/:-]/, '');
}
class ByteDanceProvider extends base_provider_js_1.BaseModelProvider {
    constructor() {
        super('bytedance');
    }
    ...
    async createImage(prompt, model, agent, opts = {}) {
        ...
        if (!apiKey)
            throw new Error('Bytedance provider: set ARK_API_KEY (or BYTEPLUS_API_KEY/BYTEDANCE_API_KEY)');
        const requestId = (0, llm_logger_js_1.log_llm_request)(agent.agent_id || 'default', 'bytedance', model, { prompt, opts }, new Date());
        try {
            ...
            const res = await (0, fetch_with_timeout_js_1.fetchWithTimeout)(`${ARK_BASE}/images/generations`, {
                method: 'POST',
                headers: {
            ...
            if (contentType.includes('text/event-stream')) {
                if (!res.body)
                    throw new Error('Bytedance: missing response body for stream');
                ...
                const flush = () => {
                    ...
                    try {
                        ...
                        if (payload?.image_base64)
                            collect(`data:image/png;base64,${payload.image_base64}`);
                        if (payload?.result?.images) {
                            for (const item of payload.result.images)
                                collect(item);
                        }
                        if (payload?.usage)
                            observedUsage = payload.usage;
                    }
                    catch (err) {
                ...
                if (Array.isArray(json?.data)) {
                    for (const d of json.data) {
                        if (d?.url)
                            out.push(String(d.url));
                        else if (d?.b64_json)
                            out.push(`data:image/png;base64,${d.b64_json}`);
                    }
                }
                if (!out.length && json?.result?.images) {
                    for (const im of json.result.images)
                        if (im?.url)
                            out.push(String(im.url));
                }
                if (!out.length && typeof json?.image_base64 === 'string') {
                    out.push(`data:image/png;base64,${json.image_base64}`);
                }
                if (json?.usage)
                    observedUsage = json.usage;
            }
            if (!out.length)
                throw new Error('Bytedance: no image result in response');
            cost_tracker_js_1.costTracker.addUsage({ model, image_count: out.length, request_id: opts?.request_id, metadata: { source: 'bytedance', usage: observedUsage } });
            (0, llm_logger_js_1.log_llm_response)(requestId, { ok: true, image_count: out.length });
            return out;
        }
        catch (err) {
...
exports.ByteDanceProvider = ByteDanceProvider;
exports.bytedanceProvider = new ByteDanceProvider();
