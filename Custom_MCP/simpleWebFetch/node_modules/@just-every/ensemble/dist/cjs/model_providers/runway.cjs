...
exports.runwayProvider = exports.RunwayProvider = void 0;
...
class RunwayProvider extends base_provider_js_1.BaseModelProvider {
    constructor() {
        super('runway');
    }
    ...
    async createImage(prompt, model, agent, opts = {}) {
        ...
        const requestId = (0, llm_logger_js_1.log_llm_request)(agent.agent_id || 'default', 'runway', model, { prompt, opts }, new Date());
        let success = false;
        try {
            if (!apiKey)
            ...
            const createRes = await withRetries(() => (0, fetch_with_timeout_js_1.fetchWithTimeout)(`${RUNWAY_BASE}/v1/text_to_image`, {
                method: 'POST',
                headers: {
            ...
            let urls = [];
            while (true) {
                const response = await withRetries(() => (0, fetch_with_timeout_js_1.fetchWithTimeout)(pollUrl, {
                    headers: {
                        Authorization: `Bearer ${apiKey}`,
                        'X-Runway-Version': RUNWAY_VERSION,
                    },
            ...
            cost_tracker_js_1.costTracker.addUsage({ model, image_count: urls.length, request_id: opts?.request_id, metadata: { source: 'runway' } });
            success = true;
            return urls;
        }
        catch (err) {
...
exports.runwayProvider = new RunwayProvider();
