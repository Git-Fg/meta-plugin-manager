...
exports.fireworksProvider = exports.FireworksProvider = void 0;
...
class FireworksProvider extends base_provider_js_1.BaseModelProvider {
    constructor() {
        super('fireworks');
    }
    ...
    fireworksModelId(model) {
        const m = model.toLowerCase();
        if (m.includes('kontext') && m.includes('max'))
    ...
    async createImage(prompt, model, agent, opts = {}) {
        ...
        const requestId = (0, llm_logger_js_1.log_llm_request)(agent.agent_id || 'default', 'fireworks', model, { prompt, opts }, new Date());
        try {
            if (!apiKey)
            ...
            if (this.isKontext(modelId)) {
                ...
                if (opts?.source_images) {
                    const s = Array.isArray(opts.source_images) ? opts.source_images[0] : opts.source_images;
                    const v = typeof s === 'string' ? s : s?.data || s;
                    if (typeof v === 'string')
                        input_image = v;
                }
                const res = await (0, fetch_with_timeout_js_1.fetchWithTimeout)(createUrl, {
                    method: 'POST',
                    headers: {
                ...
                if (!id)
                    throw new Error('Fireworks Kontext: missing request id');
                const pollUrl = `${FW_BASE}/inference/v1/workflows/accounts/fireworks/models/${modelId}/get_result`;
                const started = Date.now();
                const timeoutMs = 240000;
                while (true) {
                    const r = await (0, fetch_with_timeout_js_1.fetchWithTimeout)(pollUrl, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            Authorization: `Bearer ${apiKey}`,
                    ...
                    if (!r.ok)
                        throw new Error(`Fireworks Kontext poll failed: ${r.status} ${await r.text()}`);
                    ...
                    if (status.includes('ready') || status === 'succeeded' || status === 'completed') {
                        ...
                        if (Array.isArray(out.result?.images)) {
                            for (const im of out.result.images)
                                if (im?.url)
                                    urls.push(im.url);
                        }
                        if (out.result?.image?.url)
                            urls.push(out.result.image.url);
                        if (out.result?.url)
                            urls.push(out.result.url);
                        if (out.result?.sample)
                            urls.push(out.result.sample);
                        ...
                        if (!urls.length)
                            throw new Error('Fireworks Kontext: no image result found');
                        cost_tracker_js_1.costTracker.addUsage({ model, image_count: urls.length, request_id: opts?.request_id, metadata: { source: 'fireworks', model: modelId } });
                        return urls;
                    }
                    ...
                    if (Date.now() - started > timeoutMs)
                        throw new Error('Fireworks Kontext timed out');
            ...
            try {
                ...
                if (falKey)
                    return this.fallbackToFAL(prompt, model, opts);
                throw e;
            }
        }
        catch (err) {
            ...
            if (process.env.FAL_KEY) {
                try {
                    ...
                    (0, llm_logger_js_1.log_llm_response)(requestId, { ok: true, fallback: 'fal' });
                    return urls;
                }
                catch (_) {
                }
    ...
    async fallbackToFAL(prompt, model, opts = {}) {
        ...
        else if (lower.includes('pro') || lower.includes('kontext'))
        ...
        if (!res.ok)
            throw new Error(`FAL fallback failed: ${res.status} ${await res.text()}`);
        const data = await res.json();
        const images = [];
        ...
        if (!images.length)
            throw new Error('FAL fallback: no image url');
        cost_tracker_js_1.costTracker.addUsage({ model, image_count: images.length, request_id: opts?.request_id, metadata: { source: 'fal-fallback' } });
        return images;
    }
}
exports.FireworksProvider = FireworksProvider;
exports.fireworksProvider = new FireworksProvider();
