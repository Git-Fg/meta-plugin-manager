...
exports.midjourneyProvider = exports.MidjourneyProvider = void 0;
...
class MidjourneyProvider extends base_provider_js_1.BaseModelProvider {
    constructor() {
        super('midjourney');
    }
    ...
    async createImage(prompt, model, agent, opts) {
        ...
        if (!apiKey)
            throw new Error('Midjourney provider: MIDJOURNEY_API_KEY (or KIE_API_KEY) is not set');
        ...
        const requestId = (0, llm_logger_js_1.log_llm_request)(agent.agent_id || 'default', 'midjourney', model, { prompt, opts }, new Date());
        try {
            const aspect = mapMJAspect(opts?.size) || '1:1';
            const n = Math.max(1, Math.min(4, opts?.n || 1));
            let taskType = 'mj_txt2img';
            const fileUrls = [];
            ...
            const res = await (0, fetch_with_timeout_js_1.fetchWithTimeout)(`${KIE_BASE}/api/v1/mj/generate`, {
                method: 'POST',
                headers,
            ...
            if (!taskId)
                throw new Error('Midjourney: missing taskId');
            const timeoutMs = 300000;
            const started = Date.now();
            let images = [];
            let notFoundCount = 0;
            while (true) {
                const r = await (0, fetch_with_timeout_js_1.fetchWithTimeout)(`${KIE_BASE}/api/v1/mj/record-info?taskId=${encodeURIComponent(taskId)}`, {
                    headers: { Authorization: `Bearer ${apiKey}` },
                }, 15000);
                const info = await r.json().catch(async () => ({ code: r.status, msg: await r.text() }));
                ...
                if (code && code !== 200) {
                    if (code === 404) {
                        notFoundCount++;
                        if (notFoundCount > 5)
                            throw new Error(`Midjourney record not found for task ${taskId}`);
                    }
                    else {
                ...
                if (status === 'SUCCESS' || info?.successFlag === 1 || urls.length > 0) {
                    const urls = list
                        ...
                        .filter(Boolean);
                    if (!urls.length)
                        throw new Error('Midjourney: no result URLs');
                    images = urls.slice(0, n);
                    break;
                }
                if (info?.successFlag === 2 || status === 'FAILED')
                    throw new Error('Midjourney generation failed');
                if (Date.now() - started > timeoutMs)
                    throw new Error('Midjourney generation timed out');
                await new Promise(r2 => setTimeout(r2, 1000));
            }
            cost_tracker_js_1.costTracker.addUsage({ model, image_count: images.length, request_id: opts?.request_id, metadata: { source: 'kie' } });
            return images;
        }
        catch (err) {
...
exports.MidjourneyProvider = MidjourneyProvider;
exports.midjourneyProvider = new MidjourneyProvider();
