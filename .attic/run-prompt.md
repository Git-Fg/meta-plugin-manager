...
<mission_control>
<objective>Execute saved prompts in fresh sub-agent contexts for clean, focused execution</objective>
<success_criteria>Sub-agent spawned with prompt loaded, execution complete, results returned</success_criteria>
</mission_control>

<interaction_schema>
load_prompt → validate → spawn_subagent → collect_results → return
</interaction_schema>
...
ARCHIVE_PATH=".claude/workspace/prompts/archive/$(date +%Y/%m)/"
...
```
/run-prompt .claude/workspace/prompts/20260126_143000-add-auth.md
/run-prompt .claude/workspace/prompts/latest-prompt.md
```
...
<critical_constraint>
MANDATORY: Validate prompt has required sections before execution
MANDATORY: Spawn fresh sub-agent context for clean execution
MANDATORY: Verify results against success criteria
No exceptions. Run-prompt requires complete, validated prompts.
