...
<mission_control>
<objective>Generate structured specification-grade prompts for delegation to fresh contexts</objective>
<success_criteria>Prompt saved with all required context for autonomous sub-agent execution</success_criteria>
</mission_control>

<interaction_schema>
analyze → clarify → generate → save → execute
</interaction_schema>
...
Save the generated prompt to `.claude/workspace/prompts/` directory:

```bash
# Create prompts directory
mkdir -p .claude/workspace/prompts

# Save prompt with timestamp
PROMPT_FILE=".claude/workspace/prompts/$(date +%Y%m%d_%H%M%S)-[task-name].md"
```
...
claude --prompt "$(cat .claude/workspace/prompts/[file].md)"
```
...
mv .claude/workspace/prompts/[file].md .claude/workspace/prompts/archive/
```

**Archive structure**: `.claude/workspace/prompts/archive/YYYY/MM/[file].md`
...
```
/run-prompt .claude/workspace/prompts/[file].md
```
...
<critical_constraint>
MANDATORY: Resolve all ambiguities before generating prompt
MANDATORY: Include "What to Avoid" section with WHY explanations
MANDATORY: Make success criteria measurable and verifiable
No exceptions. Prompts must be complete enough for autonomous execution.
