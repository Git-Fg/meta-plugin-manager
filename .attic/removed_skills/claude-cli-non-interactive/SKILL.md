...
description: "Autonomous testing workflow for skills/plugins/subagents using Claude CLI print mode (-p) with stream-json output and real-time monitoring, use when testing skills/plugins/subagents in isolation for autonomous testing"
user-invocable: true
---
...
## Task-Integrated Testing Workflow

For complex test suites requiring visual progress tracking and dependency enforcement, use TaskList integration:

**When to use**:
- Multi-phase testing with 5+ steps
- Need to enforce pre-flight completion before execution
- Want visual progress tracking (Ctrl+T)
- Test suite state must persist across checkpoints

**Task Creation Pattern**:
```
Phase 1: Create test folder structure
Phase 2: Create test skills/agents [blockedBy: 1]
Phase 3: Complete pre-flight checklist [blockedBy: 2]
Phase 4: Execute test with monitoring [blockedBy: 3] ← CRITICAL: blocks until pre-flight complete
Phase 5: Analyze logs with tool-analyzer [blockedBy: 4]
Phase 6: Archive or cleanup [blockedBy: 5]
```

**Critical Value**: Phase 4 (execution) is explicitly blocked by Phase 3 (pre-flight checklist), preventing the common failure mode of skipping pre-flight validation.

**Task tracking provides**:
- Visual progression through test phases
- Dependency enforcement (execution blocked by pre-flight)
- Persistent test suite state
