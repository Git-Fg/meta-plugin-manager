...
Always write descriptions in third-person imperative.

**Why**: Descriptions are injected into the system prompt. Third-person ensures consistent semantic matching regardless of conversation context.

| Voice   | Example                                      |
| ------- | -------------------------------------------- |
| ✅ Good | "Create portable, self-sufficient skills"    |
| ✅ Good | "Analyze code patterns and generate reports" |
| ❌ Bad  | "I can help you create skills"               |
| ❌ Bad  | "You can use this to analyze code"           |

---

## Invocation Control Flags

Use frontmatter to control when Claude auto-invokes skills:

| Flag                             | Effect                         | When to Use                                |
| -------------------------------- | ------------------------------ | ------------------------------------------ |
| `disable-model-invocation: true` | Claude never auto-invokes      | High-impact operations, manual-only skills |
| `user-invocable: false`          | User cannot invoke with /skill | Background knowledge skills                |
| `allowed-tools: [tools]`         | Restrict available tools       | Read-only analysis, CI-only operations     |

**Example:**

```yaml
---
description: "Create portable skills. Use when building reusable capabilities."
disable-model-invocation: true # Manual confirmation required
---
```
...
Include keywords that users naturally say and Claude can semantically match.

**Keyword types:**

| Type    | Examples                               |
| ------- | -------------------------------------- |
| Action  | "create", "analyze", "review", "audit" |
| Domain  | "skills", "commands", "hooks", "MCP"   |
| Context | "when", "if", "during", "while"        |

**Placement:**

- Keywords near the end of description have higher weight
- Natural keyword order: action → domain → context

**Example:**
"Create portable skills. Use when building reusable capabilities, refactoring code, or when the user says 'this could be a skill'."
