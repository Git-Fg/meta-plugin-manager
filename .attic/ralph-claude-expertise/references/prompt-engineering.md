...
- Internal file paths (`.ralph/events.jsonl`)
...
instructions: |
  If approved: echo '{"topic": "approved"}' >> .ralph/events.jsonl
  If rejected: echo '{"topic": "rejected"}' >> .ralph/events.jsonl
```
...
instructions: |
  If error: echo '{"topic": "error.occurred", "data": {...}}' >> .ralph/events.jsonl
```
...
- [ ] No `.ralph/events.jsonl` file paths
...
When generating or using code task files (`.code-task.md`), follow this structured format for consistency and tracking.
...
**Note:** Read the detailed design document before beginning implementation. Read additional references as needed for context.
...
| `in_progress` | Work begins | Active implementation |
| `completed` | All criteria met | Ready for review |
| `blocked` | Cannot proceed | Waiting for dependency |

### Given-When-Then Pattern

Use this BDD-style format for acceptance criteria:
...
- Use kebab-case: `email-validator.code-task.md`
- Prefix with step number for sequences: `step01/task-01-create-models.code-task.md`
- Be descriptive but concise: `add-auth-endpoint.code-task.md`
