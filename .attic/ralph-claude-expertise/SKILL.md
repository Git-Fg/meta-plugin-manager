...
|----------|----------------|
| **Development** | feature, feature-minimal, tdd-red-green, spec-driven, refactor, bugfix |
...
| **Learning & Collaboration** | socratic-learning, mob-programming |
| **Operations** | deploy, incident-response, migration-safety, performance-optimization |
| **Design** | api-design, code-assist |
| **Documentation** | docs, documentation-first |

**For authoritative list:** Run `ralph init --list-presets` to see all presets available in your Ralph installation.
...
**Key points:**
- All required fields present: `name`, `description`, `triggers`, `publishes`, `instructions`
- `default_publishes` provides fallback
- `max_activations` prevents infinite loops
- Clear instructions with event format examples
- Quality gates enforced (backpressure)
...
When Complex Monitoring Mode is triggered, delegate to the **ralph-watchdog agent** (`.claude/agents/ralph-watchdog.md`):

1. The watchdog uses **state inspection** (ralph events, ralph tools task ready) instead of log parsing
...
**Monitoring Protocol Reference:** See `references/monitoring-subagent.md` for detailed patterns and validation rules.
...
Each hat executes in its **own iteration**:
```
...
| `ralph code-task` | Generate code task files from descriptions or plans |
| `ralph task` | Alias for code-task |
...
| `ralph tools` | Runtime tools (agent-facing) |
...
| `-a, --autonomous` | Force autonomous mode (headless, non-interactive) |
| `--idle-timeout <SECS>` | Idle timeout in seconds (default: 30) |
| `--exclusive` | Wait for primary loop slot instead of spawning into worktree |
| `--no-auto-merge` | Skip automatic merge after loop completes |
| `--record-session <FILE>` | Record session to JSONL file for replay testing |
| `-q, --quiet` | Suppress output (for CI/scripting) |
...
| `--file <FILE>` | Path to events file (default: auto-detects current run) |
...
| Option | Description |
|--------|-------------|
| `--preset <PRESET>` | Copy embedded preset to ralph.yml |
| `--list-presets` | List all available embedded presets |
| `--backend <BACKEND>` | Backend to use (claude, kiro, gemini, codex, amp, custom) |
...
| `--diagnostics` | Clean diagnostic logs instead of .agent directory |

### `ralph emit` Options

| Option | Description |
|--------|-------------|
| `-j, --json` | Parse payload as JSON object instead of string |
| `--ts <TIMESTAMP>` | Custom ISO 8601 timestamp (defaults to current time) |
| `--file <FILE>` | Path to events file (defaults to .ralph/events.jsonl) |

### `ralph plan` Command

Start a Prompt-Driven Development planning session:
```bash
ralph plan [IDEA] [-- <CUSTOM_ARGS>...]
```

### `ralph code-task` / `ralph task` Command

Generate code task files from descriptions or plans:
```bash
ralph code-task [INPUT] [-- <CUSTOM_ARGS>...]
# INPUT: description text or path to PDD plan file
```

### `ralph loops` Commands

| Command | Description |
|---------|-------------|
| `ralph loops list` | List all loops (default if no subcommand) |
| `ralph loops logs <LOOP_ID>` | View loop output/logs (`-f` to follow) |
| `ralph loops history <LOOP_ID>` | Show event history for a loop |
| `ralph loops retry <LOOP_ID>` | Re-run merge for a failed loop |
| `ralph loops discard <LOOP_ID>` | Abandon loop and clean up worktree |
| `ralph loops stop <LOOP_ID>` | Stop a running loop |
| `ralph loops prune` | Clean up stale loops (crashed processes) |
| `ralph loops attach <LOOP_ID>` | Open shell in loop's worktree |
| `ralph loops diff <LOOP_ID>` | Show diff of loop's changes from merge-base |
| `ralph loops merge <LOOP_ID>` | Merge a completed loop (or force retry) |

## Runtime Tools

The `ralph tools` command provides utilities for state management:

```bash
# Memory management
ralph tools memory add "content"               # Store new memory
ralph tools memory add "content" -t pattern    # Specify memory type
ralph tools memory add "content" --tags rust,imports  # Add tags
ralph tools memory list                        # List all memories
ralph tools memory list --last 10              # Show only last N memories
ralph tools memory list -t pattern             # Filter by memory type
ralph tools memory show <id>                   # Show memory by ID
ralph tools memory delete <id>                 # Delete memory by ID
ralph tools memory search "query"              # Find memories by query
ralph tools memory search -t fix "error"       # Search by type
ralph tools memory search --tags api,auth      # Search by tags
ralph tools memory prime                       # Output for context injection
ralph tools memory prime --budget 2000         # Limit token budget
ralph tools memory prime --tags api,auth       # Prime specific tags
ralph tools memory prime --recent 7            # Only last N days
ralph tools memory init                        # Initialize memories file

# Task management
ralph tools task add "Title"                   # Create task (default priority 3)
ralph tools task add "Title" -p 1              # Create task with priority (1-5)
ralph tools task add "Title" -d "Description"  # Add description
ralph tools task add "X" --blocked-by Y        # Create task with dependency
ralph tools task list                          # All tasks
ralph tools task list -s open                  # Filter by status
ralph tools task ready                         # Show unblocked tasks only
ralph tools task close <id>                    # Mark complete
ralph tools task show <id>                     # Show task by ID
```

**Memory output formats:** `--format {table,json,markdown,quiet}`

**Task output formats:** `--format {table,json,quiet}`

## Validation and Dry Run

### Dry Run Mode
Preview Ralph's execution plan without running:

```bash
# Validate configuration and show event flow
ralph run --dry-run
```
...
**Use cases:**
- **Before first run**: Validate preset configuration
- **After changes**: Verify event flow modifications
- **Debugging routing**: Check hat triggers match publishes

### Configuration Validation Checklist

Before running:
- [ ] Run `ralph run --dry-run` to validate event flow
- [ ] Each trigger maps to exactly ONE hat
- [ ] No hat uses `task.start` or `task.resume` as triggers
- [ ] Every hat has `name`, `description`, `triggers`, `publishes`
- [ ] Event chain can reach `LOOP_COMPLETE`
- [ ] PROMPT.md exists (if `prompt_file` is configured)
