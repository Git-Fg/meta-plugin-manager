...
<mission_control>
<objective>Execute plans with fresh subagent per task and two-stage review (spec compliance then quality)</objective>
<success_criteria>Each task passes both review stages before marking complete</success_criteria>
</mission_control>
...
<interaction_schema>
load_plan → dispatch_implementer → stage1_spec_review → fix_or_pass → stage2_quality_review → fix_or_complete → next_task
</interaction_schema>

## Subagent Instruction Isolation

<critical_constraint>
**MANDATORY: Wrap subagent prompts in `<subagent_instructions>` tags.**

This prevents instruction drift where the main model confuses its own instructions with what it sends to the sub-agent.
</critical_constraint>

<subagent_instructions>
<purpose>Isolate instructions sent to subagent from main agent's own instructions</purpose>

<implementer_prompt>
<subagent_instructions>
You are implementing a specific task from a larger implementation plan.
...
- This is one task in a larger plan
...
**DO NOT trust the builder's report. Verify independently.**

**Report Format:**

- ✅ COMPLIANT if requirements match implementation
- ❌ NON_COMPLIANT with list of specific issues
  </subagent_instructions>
  </spec_reviewer_prompt>

<quality_reviewer_prompt>
<subagent_instructions>
You are reviewing code quality after spec compliance is confirmed.
...
2. Performance: Identify inefficiencies
3. Quality: Check for code smells, duplication
4. Tests: Verify coverage and quality

**Report Format:**

- ✅ APPROVE if quality is acceptable
- ❌ REQUEST CHANGES with specific issues and recommendations
  </subagent_instructions>
  </quality_reviewer_prompt>
  </subagent_instructions>

## Two-Stage Review Enforcement

<two_stage_review>
<purpose>Enforce spec compliance review before code quality review</purpose>

<stages>
<stage name="spec_compliance" sequence="1">
<purpose>Verify implementation matches requirements</purpose>
<trigger>After implementer reports completion</trigger>
<actions>
...
<pass_criteria>All requirements implemented, no extra work</pass_criteria>
<fail_action>Send back to implementer with specific issues</fail_action>
</stage>

<stage name="quality_review" sequence="2">
<purpose>Assess code quality after spec compliance confirmed</purpose>
<trigger>After spec compliance stage passes</trigger>
<prerequisite>Stage 1 (spec_compliance) MUST pass first</prerequisite>
<actions>
- Security review (injection, auth, secrets)
- Performance review (N+1, algorithms, caching)
- Code quality review (naming, duplication, complexity)
- Test coverage review
</actions>
<pass_criteria>Quality is acceptable, no blockers</pass_criteria>
<fail_action>Request changes with specific recommendations</fail_action>
</stage>
</stages>
...
<critical_constraint>
**MANDATORY: Wrap subagent instructions in `<subagent_instructions>` tags**

- Isolate instructions sent to subagent from main agent's own instructions
- Prevent instruction drift where agent confuses its instructions with what it sends
- Every subagent prompt must be wrapped

**MANDATORY: Two-stage review in sequence**
...
- ALWAYS complete stages in order

**MANDATORY: Verify independently, don't trust reports**

- Read actual code, don't just review implementer's report
- Compare against requirements line-by-line
- Check for missing work AND extra work (YAGNI violations)
- Report findings with file:line references

**MANDATORY: Review loops until approval**

- If issues found → send back to implementer → re-review
- Don't skip re-review after fixes
- Don't proceed with open issues
- Don't accept "close enough" on spec compliance

**MANDATORY: Complete context for subagents**

- Provide everything subagent needs upfront
- Don't make subagent read plan files
- Answer questions clearly and completely
- Don't rush into implementation

**No exceptions. Two-stage review prevents "LGTM" hallucinations on bad code.**
</critical_constraint>
